...
__int64 WinPmem::extract_file_(__int64 resource_id, TCHAR *filename) {
  // Locate the driver resource in the .EXE file.
	HRSRC hRes = FindResource(g_dllModule, MAKEINTRESOURCE(resource_id), L"FILE");
  if (hRes == NULL) {
    LogError(TEXT("Could not locate driver resource."));
    goto error;
  }

  HGLOBAL hResLoad = LoadResource(g_dllModule, hRes);
  if (hResLoad == NULL) {
  ...
  DWORD size = SizeofResource(g_dllModule, hRes);
