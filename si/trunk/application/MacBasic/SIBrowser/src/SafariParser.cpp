...
#include "SIBrowser/include/SIBrowser/SafariCookieDef.h"
...
namespace  macbasic
{
    ...
    bool CSafariParser::ParsePlistHistory(std::wstring historyFile, BrowserHistorys& historys)
    {
        ...
        for(; it_a != arr.end(); ++it_a)
        {
            ...
            if(it_dc != dict_c.end())
            {
                ...
                newHistory.visit_time_ = CPTime::UTC2LocalTime(ttime, siutility::MacCommonHelper::GetTimezoneRegion());
            }
    ...
    bool CSafariParser::ParseDbHistory(std::wstring historyFile, BrowserHistorys& historys)
    {
        ...
        while(sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            newHistory.visit_time_ = CPTime::UTC2LocalTime(tvt, siutility::MacCommonHelper::GetTimezoneRegion());
    ...
    bool CSafariParser::ParseDownload(std::wstring downloadPlist, BrowserDownloads& downloads)
    {
        ...
        for(; it_a != arr.end(); ++it_a)
        {
            ...
            if(it_dc != dict_c.end())
            {
                uint64 loadsize = boost::any_cast<plistcpp::integer_type>(it_dc->second);
                newDownload.loaded_size_ = siutility::AutoUnitConvert::BytesToAutoUint(loadsize);
            }
            it_dc = dict_c.find("DownloadEntryProgressTotalToLoad");
            if(it_dc != dict_c.end())
            {
                uint64 filesize = boost::any_cast<plistcpp::integer_type>(it_dc->second);
               newDownload.file_size_ = siutility::AutoUnitConvert::BytesToAutoUint(filesize);
            }
            ...
            if(it_dc != dict_c.end())
            {
                ...
                newDownload.start_time_ = CPTime::UTC2LocalTime(tdt, siutility::MacCommonHelper::GetTimezoneRegion());
            }
            ...
            if(it_dc != dict_c.end())
            {
                plistcpp::date_type dateTime = boost::any_cast<plistcpp::date_type>(it_dc->second);
                time_t tdt = dateTime.timeAsEpoch();
                newDownload.finish_time_ =CPTime::UTC2LocalTime(tdt, siutility::MacCommonHelper::GetTimezoneRegion());
            }
    ...
    bool CSafariParser::ParseCache(std::wstring cacheFile, BrowserCaches& caches)
    {
        ...
        while(SQLITE_ROW == sqlite3_step(pStmt))
        {
            ...
            safariCache.visit_time_ = CPTime::UTC2LocalTime(tvt, siutility::MacCommonHelper::GetTimezoneRegion());
    ...
    time_t CSafariParser::TimestrToTimet(const unsigned char* strTime)
    {
        if (NULL == strTime)
        {
            return 0;
    ...
    bool CSafariParser::ParseBookmark(std::wstring bookmarkFile, BookMarkPtr bookmark)
    {
    ...
    void CSafariParser::BookmarkParseRecursive(plistcpp::dictionary_type& dict, BookMarkPtr bookmark)
    {
    ...
    bool CSafariParser::ParseCookie(std::wstring cookieFile, BrowserCookies& cookies)
    {
        ...
        std::vector<SAFARI_COOKIE_PAGE> CookiePages; //! cookies ҳ����
        ...
        for (uint i = 0;i<CookiePages.size();i++)
        {
            ...
            for (uint j=0;j<RecordCount;j++)
            {
                ...
                newCookie.expireTime_ = CPTime::UTC2LocalTime(expTime, siutility::MacCommonHelper::GetTimezoneRegion());

                //! ��������ʱ��
                time_t createTime = MAC_BASETIME ;//+ (uint64)record->LastAccess;
                newCookie.createdTime_ = CPTime::UTC2LocalTime(createTime, siutility::MacCommonHelper::GetTimezoneRegion());
                //! ����
    ...
    bool CSafariParser::GetHistory(CPRecord *rootRecord)
    {
        ...
        siutility::MacCommonHelper::GetHandlesByUserRawPath(L"Library/Safari", L"History.plist", plistHandles);
        ...
        siutility::CMacFileExport mfx(L"Library/Safari/", L"History.db");
    ...
    bool CSafariParser::GetDownload(CPRecord *rootRecord)
    {
        //Safari 6.0��Safari 8.0��/Users/�û���/Library/Safari/Downloads.plist

        std::vector<PSIHANDLE> plistHandles;
        siutility::MacCommonHelper::GetHandlesByUserRawPath(L"Library/Safari", L"Downloads.plist", plistHandles);
        ...
        CPRecordType* chromeType = CPRecordTypeFactory::get()->getType(L"table.browserdownload");
            CPRecord* osRootRecord = chromeType->newRecord();
            rootRecord->addChild(osRootRecord);
    ...
    bool CSafariParser::GetCache(CPRecord *rootRecord)
    {
        ...
        siutility::CMacFileExport cmfe(L"/Library/Caches/com.apple.Safari/", L"Cache.db");
        ...
        CPRecordType* chromeType = CPRecordTypeFactory::get()->getType(L"table.browsercache");
            CPRecord* osRootRecord = chromeType->newRecord();
            rootRecord->addChild(osRootRecord);
    ...
    bool CSafariParser::GetBookmark(CPRecord *rootRecord)
    {
       ...
       // 10.8��10.10��/Users/�û���/Library/Caches/Metadata/Safari/Bookmarks/*.webbookmark��
        ...
        std::vector<PSIHANDLE> plistHandles;
        siutility::MacCommonHelper::GetHandlesByUserRawPath(L"Library/Safari/", L"Bookmarks.plist", plistHandles);
        ...
        {
            std::wstring plistPath = che.Export(*it_h);
            BookMarkPtr oneBookMark = boost::make_shared<CBookmark>();
        ...
        CPRecordType* chromeType = CPRecordTypeFactory::get()->getType(L"table.browserbookmark");
            CPRecord* osRootRecord = chromeType->newRecord();
            rootRecord->addChild(osRootRecord);
    ...
    bool CSafariParser::GetCookie(CPRecord *rootRecord)
    {
        ...
        siutility::MacCommonHelper::GetHandlesByUserRawPath(L"Library/Cookies/", L"com.apple.Safari.SafeBrowsing.binarycookies", tempHandles1);
        siutility::MacCommonHelper::GetHandlesByUserRawPath(L"Library/Cookies/", L"Cookies.binarycookies", tempHandles2);
        ...
        {
            std::wstring cookiePath = che.Export(*it_h);
            ParseCookie(cookiePath, cookies);
        }

        CPRecordType* chromeType = CPRecordTypeFactory::get()->getType(L"table.browsercookie");
            CPRecord* osRootRecord = chromeType->newRecord();
            rootRecord->addChild(osRootRecord);
