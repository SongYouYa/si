#include "stdafx.h"
#include "SystemInfo/include/systeminfo/autorun/AutorunParser.h"
#include "SystemInfo/include/systeminfo/autorun/LaunchParser.h"
...
namespace macbasic {
    ...
    void CAutorunParser::GetAutorunInfo(CPRecord *table)
    {
        CPRecordType* osType = CPRecordTypeFactory::get()->getType(L"table.autorun");
        CPRecord* diskRecord = osType->newRecord();
        table->addChild(diskRecord);
        ...
        {
           it_r->AddToTable(diskRecord);

        }
        return;
    }

    bool CAutorunParser::ParseKext(PSIHANDLE plistHandle, AutoRunInfo& kextInfo)
    {
        siutility::CHandleExport che;
        std::wstring plistPath = che.Export(plistHandle);
    ...
    void CAutorunParser::GetKext(AutoRunInfos& allKextInfo)
    {
        std::vector<PSIHANDLE> pInfoHandles;
        siutility::CFileSearch cfs(MAC_AUTORUN_KEXT_ROOT, L"/Info.plist$");
    ...
    void CAutorunParser::GetLaunchctl(AutoRunInfos& launchctlInfos)
    {
    ...
    bool CAutorunParser::ParseLaunchctl(PSIHANDLE handle, AutoRunInfo& lauchInfo)
    {
     ...
     void CAutorunParser::GetMachInit(AutoRunInfos& machinitInfos)
     {
        std::wstring rootPath = L"/etc/mach_init.d/";
        siutility::CFileSearch cfs(rootPath, L".*");
     ...
     bool CAutorunParser::ParseMachInit(PSIHANDLE plistHandle, AutoRunInfo& machinitInfo)
     {
        siutility::CHandleExport che;
        std::wstring plistPath = che.Export(plistHandle);
        std::string strPath = CPCodeChanger::UnicodeToUTF8(plistPath.c_str());
     ...
     void CAutorunParser::GetLauchDaemons(AutoRunInfos& lauchDaemonsInfos)
     {
        std::vector<PSIHANDLE> plistHandles;
        std::vector<PSIHANDLE> tempHandles1;
        std::vector<PSIHANDLE> tempHandles2;
        siutility::CFileSearch cfs(L"/Library/LaunchDaemons/", L".plist");
        cfs.GetHandles(tempHandles1);
        siutility::CFileSearch cfs2(L"/System/Library/LaunchDaemons/", L".plist");
     ...
     void CAutorunParser::GetSystemStarter(AutoRunInfos& systemStarterInfos)
     {
     ...
     void CAutorunParser::GetSystemStarterProg(AutoRunInfos& systemStarterInfos)
     {
     ...
     void CAutorunParser::ParseSystemStarterProg(std::map<std::wstring, std::wstring> progparams, AutoRunInfos& systemStarterInfos)
     {
     ...
     void CAutorunParser::GetSystemStarterScript(AutoRunInfos& systemStarterInfos)
     {
         std::vector<PSIHANDLE> handles;
     ...
     void CAutorunParser::GetSystemStarterItems(AutoRunInfos& systemStarterInfos)
     {
     ...
     bool CAutorunParser::ParseSystemStarterItems(PSIHANDLE rootHandle, AutoRunInfos& systemStarterInfos)
     {
         std::vector<PSIHANDLE> subHandles;
         filesystem::CFileSystem::GetAllSubItems(rootHandle, false, L"", &subHandles);
         if(subHandles.empty())
         {
             return false;
         }
     ...
     void CAutorunParser::GetLaunchAgent(AutoRunInfos& agentInfos)
     {
        ...
        siutility::MacCommonHelper::GetMacAllUserName(userNames);
     ...
     bool CAutorunParser::ParseLaunchAgentRoot(PSIHANDLE rootHandle, AutoRunInfos& agentInfos)
     {
         std::vector<PSIHANDLE> subHandles;
         CFileSystem::GetAllSubItems(rootHandle,  false, L"", &subHandles);
         for(std::vector<PSIHANDLE>::iterator it = subHandles.begin(); it != subHandles.end(); ++it)
         {
             CLaunchParser clp;
             AutoRunInfo oneInfo;
     ...
     void CAutorunParser::GetLoginitems(AutoRunInfos& loginInfos)
     {
         std::vector<PSIHANDLE> plistHandles;
         siutility::MacCommonHelper::GetHandlesByUserRawPath(L"/Library/Preferences/", L"com.Apple.loginitems.plist", plistHandles);
         for(std::vector<PSIHANDLE>::iterator it = plistHandles.begin(); it != plistHandles.end(); ++it)
         {
            AutoRunInfos tempInfos;
     ...
     bool CAutorunParser::ParseLoginitem(PSIHANDLE plistHandle, AutoRunInfos& loginInfos)
     {
         siutility::CHandleExport che;
         std::wstring plistPath = che.Export(plistHandle);
         std::string strPath = CPCodeChanger::UnicodeToUTF8(plistPath.c_str());
