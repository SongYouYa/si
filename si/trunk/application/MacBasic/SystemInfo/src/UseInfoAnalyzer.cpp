...
UseInfoAnalyzer::UseInfoAnalyzer()
{
...
bool UseInfoAnalyzer::GetUseInfo(CPRecord *rootRecord)
{
    ...
    for (std::vector<PSIHANDLE>::iterator iter = groupPlistHandles.begin(); iter != groupPlistHandles.end(); ++iter)
    {
        ...
        std::wstring plistPath = che.Export(groupHandle);
    ...
    for (std::vector<PSIHANDLE>::iterator iter = userPlistHandles.begin(); iter != userPlistHandles.end(); ++iter)
    {
        ...
        std::wstring plistPath = che.Export(userHandle);
        ReadUserPlistFile(plistPath,userHandle);
    }
    CPRecordType* osType = CPRecordTypeFactory::get()->getType(L"table.userinfo");
    CPRecord* diskRecord = osType->newRecord();
    rootRecord->addChild(diskRecord);
    WriteDataInTable(diskRecord,m_vecUser);
    return true;
}

bool UseInfoAnalyzer::ReadGroupPlistFile(std::wstring plistPath)
{
...
bool UseInfoAnalyzer::ReadUserPlistFile(std::wstring plistPath,PSIHANDLE handle)
{
    USER user;
    std::string strFilePath = CPCodeChanger::UnicodeToUTF8(plistPath.c_str());
    plistcpp::dictionary_type dict;
...
std::wstring UseInfoAnalyzer::GetGroupName(const std::wstring &wstrUserName)
{
...
std::wstring UseInfoAnalyzer::GetUserType(USER user)
{
...
GROUP *UseInfoAnalyzer::GetGroupByGID(const std::wstring wstrGID)
{
...
bool UseInfoAnalyzer::findUserInGroup(const std::wstring wstrUserName, GROUP *pGroup)
{
...
void UseInfoAnalyzer::WriteDataInTable( CPRecord* diskRecord,std::vector<USER> &vecUser)
{  
    // table.userinfo
    ...
    for(unsigned int i = 0; i < vecUser.size(); i++)
    {
        ...
        osRecord->at(L"SIUserID") = vecUser[i].wstrUID;
        osRecord->at(L"SIUseName") = vecUser[i].wstrUserName;
        osRecord->at(L"SIUseType") = vecUser[i].wstrUserType;
        osRecord->at(L"SIGroupID") = vecUser[i].wstrGID;
        osRecord->at(L"SIGroupName") = vecUser[i].wstrGroupName;
        osRecord->at(L"SIMainDire") = vecUser[i].wstrHome;
        osRecord->at(L"SICreateTime") = vecUser[i].timeCreated;
        osRecord->at(L"SIFileVaultDire") = vecUser[i].wstrHomeLoc;
        osRecord->at(L"SIReMark") = vecUser[i].wstrRealName;
    }
}
}
