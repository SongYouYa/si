...
//#include "PCodeChanger.h"
//#include "windows_type.h"
int memicmp(const char * pStr1,
            const char * pStr2, size_t size);
int stricmp(const char * pStr1, const char * pStr2);



static const int MESSAGE_MAX_SIZE = 50 * 1024 * 1024;
...
public:
    ...
    void GetValue(string& strValue, bool bAll=false) const;
...
public:
	...
	void GetFieldValue(const char* pszFieldName, string & strFieldValue, bool bAll=false) const;
...
#define TABLE_SIZE
	...
	static const MediaTypeCvt m_TypeCvtTable[TABLE_SIZE];
	static const char* m_TypeTable[TABLE_SIZE];
...
inline void CMimeHeader::GetFieldValue(const char* pszFieldName, string & strFieldValue, bool bAll) const
{
	const CMimeField* pfd = GetField(pszFieldName);
	if (pfd != NULL)
	{
		pfd->GetValue(strFieldValue, bAll);
	}
    else
    {
        strFieldValue = "";
...
public:
    ...
    void DeleteAll();
    CMimeBody* CreatePart(CMimeBody* pWhere=NULL);
...
protected:
    CMimeHeader	m_mimeHeader;		// header of the body part
...
inline CMimeHeader & CMimeBody::GetMimeHeader()
{
    return m_mimeHeader;
}
...
{ return m_mimeHeader.GetMediaType() == CMimeHeader::MEDIA_TEXT; }

inline bool CMimeBody::IsMessage() const
{ return m_mimeHeader.GetMediaType() == CMimeHeader::MEDIA_MESSAGE; }

inline bool CMimeBody::IsAttachment() const
{ return m_mimeHeader.GetName().size() > 0; }

inline bool CMimeBody::IsMultiPart() const
{ return m_mimeHeader.GetMediaType() == CMimeHeader::MEDIA_MULTIPART; }
...
public:
    ...
    void SetFrom(const string & strFrom, const string & strCharset="");
    void GetFrom(string & strFrom) const;
    void SetTo(const string & strTo, const string & strCharset="");
    void GetTo(string & strTo) const;
    void SetCc(const string & strCc, const string & strCharset="");
    void GetCc(string & strCc) const;
    void SetBcc(const string & strBcc, const string & strCharset="");
    void GetBcc(string & strBcc) const;

    void SetSubject(const string & strSubject, const string & strCharset="");
    void GetSubject(string & strSubject) const;
    void SetContentBase(const string & strSubject, const string & strCharset="");
    void GetContentBase(string & strSubject) const;

    void GetReceived(string & strReceived) const;
    void SetDeliveredTo(const string & strTo, const string & strCharset="");
    void GetDeliveredTo(string & strTo) const;
    void SetRSSArticleUrl(const string & strFrom, const string & strCharset="");
    void GetRSSArticleUrl(string & strFrom) const;

    
    // set default date as current time
    void SetDate();
    void SetDate(int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond);
    // get date string (format: Thu, 17 Dec 2009 17:44:09 +0800)
    void GetDate(string & strDate) const;
    // set default Mime-Version: 1.0
    void SetVersion();

    void SetFieldValue(const string & strFieldName, const string & strFieldValue, const string & strCharset="");
    void GetFieldValue(const string & strFieldName, string & strFieldValue) const;

    // get the text (plain/text) of the child body
    void GetPlainText(string & strPlainText) const;
    void GetHtmlText(string & strHtmlText) const;

    // get the list of attachment
    void GetAttachmentList(vector< pair<string, int> > & arrAttList) const;

    // export the attachment to full path
    bool ExportAttachment(const string & strFullPath, unsigned int index);

    // set the child body of message (eg. plain text. html text. attachment)
    bool SetChildBody(const string & strPlainText="", const string & strHtmlText="", const vector<string> & arrAttPath=vector<string>());
    ...
    bool ExportMessage(const string & strFullPath);
...
protected:
    CMimeBody * m_pRootBody;
};

inline void CMimeMessage::SetFrom(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("From", strAddr, strCharset);
}

inline void CMimeMessage::GetFrom(string & strFrom) const
{ 
    GetFieldValue("From", strFrom);
}

inline void CMimeMessage::SetTo(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("To", strAddr, strCharset);
}

inline void CMimeMessage::GetTo(string & strTo) const
{
    GetFieldValue("To", strTo);
}

#ifndef SI_MAC

inline void CMimeMessage::SetDeliveredTo(const string & strTo, const string & strCharset)
{ 
    SetFieldValue("Delivered-To", strTo, strCharset);
}

inline void CMimeMessage::GetDeliveredTo(string & strTo) const
{
    GetFieldValue("Delivered-To", strTo);
}

inline void CMimeMessage::SetRSSArticleUrl(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("X-Mail-Rss-Article-Url", strAddr, strCharset);
}

inline void CMimeMessage::GetRSSArticleUrl(string & strFrom) const
{ 
    GetFieldValue("X-Mail-Rss-Article-Url", strFrom);
}


#endif

inline void CMimeMessage::SetCc(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("Cc", strAddr, strCharset);
}

inline void CMimeMessage::GetCc(string & strCc) const
{ 
    GetFieldValue("Cc", strCc);
}

inline void CMimeMessage::SetBcc(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("Bcc", strAddr, strCharset);
}

inline void CMimeMessage::GetBcc(string & strBcc) const
{ 
    GetFieldValue("Bcc", strBcc);
}

inline void CMimeMessage::SetSubject(const string & strSubject, const string & strCharset)
{ 
    SetFieldValue("Subject", strSubject, strCharset);
}

inline void CMimeMessage::GetSubject(string & strSubject) const
{ 
    GetFieldValue("Subject", strSubject);
}

inline void CMimeMessage::SetContentBase(const string & strAddr, const string & strCharset)
{ 
    SetFieldValue("Content-Base", strAddr, strCharset);
}

inline void CMimeMessage::GetContentBase(string & strFrom) const
{ 
    GetFieldValue("Content-Base", strFrom);
}

inline void CMimeMessage::GetDate(string & strDate) const
{ 
    GetFieldValue("Date", strDate);
}
